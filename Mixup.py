...
def rand_bbox(image_shape, lam, count=None):
    ...
    # get bbox coords
    # 返回的是四个标量，用clip函数防止索引越界，像素位置超出图片大小
...
def one_hot(x, num_classes, on_value=1., off_value=0.):
    ...
    """
    x = x.reshape_([-1, 1])
    x_smoothed = torch.full((x.shape[0], num_classes), fill_value=off_value)
...
if __name__ == '__main__':
    a = torch.arange(3*4*4).view(3,4,4)
    b, c, d, e = rand_bbox([3, 256, 256], 0.9)
    print(b)
    print(c)
    print(d)
    print(e)